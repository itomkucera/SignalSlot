...
class Signal {
   public:
    Signal() : impl_{std::make_shared<detail::SignalImpl<Args...>>()} {}
    ...
    // function - non-static member function of disconnector, invocable with
    // Args... disconnector - inherits Disconnector
    template <typename S, typename D>
    Connection Connect(
        S&& function, D* terminator,
        typename std::enable_if_t<std::is_base_of_v<AutoTerminator, D> &&
                                  std::is_invocable_v<S, D, Args...>>* =
            nullptr) {
        // capture by value - slot should always be a pointer
        // void(D::*f)(Args...) doesn't expect const - another overload would be
        // needed
        return Connect(
            [=](Args&&... args) {
    ...
    template <typename S>
    Connection Connect(
        S&& function, AutoTerminator* terminator,
    ...
    void Emit(EmitArgs&&... args) const {
        static_assert((std::is_convertible_v<EmitArgs, Args> && ...),
                      "Parameter types don't match.");

        impl_->Emit(std::forward<EmitArgs>(args)...);
    }

    bool GetSlotCount() const { return impl_->GetSlotCount(); }

    void TerminateAll() { impl_->TerminateAll(); }
